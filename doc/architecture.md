...
Any successfully decoded `FsmEvent::RecvMessage` causes the FSM to emit `FsmAction::ResetHeartbeat`, which the runtime uses to reschedule both the send-heartbeat timer and the missed-heartbeat deadline. This way the rule "any valid message resets the timer" lives in exactly one place. Two consecutive missed intervals trigger a Session Termination with status code 132.
...
### 5.9 TLS is wired but not yet enabled by default

`use_tls` defaults to `false` in `NetworkConfig::default()` while the TLS connect/accept path is still a stub. The default flips to `true` when M7 lands (rustls `Connector`/`Acceptor` wired end-to-end). Until then, requesting TLS at runtime returns a typed `io::Error` rather than panicking via `todo!()`.
